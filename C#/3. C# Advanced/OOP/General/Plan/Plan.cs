...
class Plan
{
    static void Main(string[] args)
    {
        ...
        Console.WriteLine("<--------Unit тестване-------->"); // Тестването зависи от контекста - няма еднозначно решение; Пълно тестване е невъзможно, защото всички комбинации на едно приложение
                                                              // от входни данни и потенциални изходни данни са безкрайни; Анализира се риска кое е най-важно да работи и се приотизира; Колкото -
                                                              // по-рано тестваме, толкова по-малък е шанса да пуснем нещо с бъгове - първо пишем тестовете за приложението, а след това
                                                              // функционалността; Тестове, които се повтарят стават неефективти; С подобряването на приложението се пишат и нови тестове; 
                                                              // Тестовете не означават приложение без бъгове; Може да има дефекти, които тестовете да не хващат; Съществуват различни видове
                                                              // тестове: 1. Unit - тестване на компонент - NUnit, JUnit, PyUnit, Mocha, 2. Integration - тестване на интеграция между отделни
                                                              // компоненти - отнемат повече време, по-бавни са, не тества по-задъблочено - API *най-добър вариант, когато не ни се занимава да
                                                              // отделяме много време за тестване*, 3. System - тестване на система как работи от начало до край - Selenium, Appium, Cypress;
                                                              // AAA pattern - Arrange(подготвяме входните данни) -> Act(извикваме операцията за тестване) -> Assert(проверяваме изходните
                                                              // данни и уславия); Как да пишем добри тестове: 1. Имената на тестовете трябва да описват какво точно тестват, 2. Колкото пъти
                                                              // пуснем тест, той трябва да връща едни и същи резултати - резултатите трябва да бъдат твърдо зададени, ясни и точни, 3. Не трябва
                                                              // да зависят от други тестове, 4. Входните данни се задават в Arrange фазата, 5. Могат да зависят единствено от [SetUp]
                                                              // инициализацията, 6. Ако има нужда да се чистят ресурси, изчистват се, 7. Всеки тест тества съответен случай
    }
