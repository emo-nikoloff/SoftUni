...
Take every digit from the string and store it somewhere. After that, remove all the digits from the string. After this operation, you should have two lists of items: the numbers list and the
non-numbers list:
...
· Take list: [0, 4, 7]
· Skip list: [4, 1, 0]
Afterward, iterate over both lists and skip {skipCount} characters from the non-numbers list, then take {takeCount} characters and store it in a result string. Note that the skipped characters are
...
class Program
{
    static void Main(string[] args)
    {
        ...
        List<int> messageDigits = new();
        List<char> messageString = new();
        ...
        for (int i = 0; i < message.Length; i++)
        {
            ...
            if (isDigit)
            {
                messageDigits.Add(message[i]);
            }
            else
            {
                messageString.Add(message[i]);
            }
        }

        List<int> takeList = new(), skipList = new();

        for (int i = 0; i < messageDigits.Count; i++)
        {
            if (i % 2 == 0)
            {
                takeList.Add(messageDigits[i]);
            }
            else if (i % 2 != 0)
            {
                skipList.Add(messageDigits[i]);
            }
        }
