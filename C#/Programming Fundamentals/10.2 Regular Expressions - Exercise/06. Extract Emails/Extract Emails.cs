/*Write a program to extract all email addresses from a given text. The text comes at the only input line. Print the emails on the console, each at a separate line. Emails are considered to be in
format <user>@<host>, where:
...
class Program
{
    static void Main(string[] args)
    {
        ...
        string pattern = @"(?<=\s|^)(?<user>[A-Za-z0-9]+(?:[._-][A-Za-z0-9]+)*)@(?<host>[A-Za-z0-9]+(?:[.-][A-Za-z0-9]+)*\.[A-Za-z]{2,})";
