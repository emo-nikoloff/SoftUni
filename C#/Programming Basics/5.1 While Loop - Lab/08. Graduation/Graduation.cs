...
int failsCount = 0;
double sumGrades = 0;
while (graduatedFrom < 12 && failsCount < 2)
{
...
if (graduatedFrom == 12)
{
    Console.WriteLine($"{name} graduated. Average grade: {sumGrades / graduatedFrom:f2}");
}
else
{
    Console.WriteLine($"{name} has been excluded at {graduatedFrom + 1} grade");
}